...
| `Ext.StaticData` | ✅ Full (5) | ✅ **10 types** (Feat, Race, Background, Origin, God, Class, Progression, ActionResource, FeatDescription, CharacterCreationAppearanceVisual), ForceCapture, HashLookup, typed field layouts | **100%** | 10 |
...
- [x] Integration tests with mock game state (368 pytest Tier H tests, including harness lifecycle, offset audit, physics VMT audit + RaycastAny gating guard, doctor toolchain checks, and compat scenarios)
- [x] Regression test suite (Wave 7 A/B/C-series, 2026-08-03: 544-test four-tier suite — 65 C Tier 0 + 255 pytest Tier H + 114 Tier 1 + 110 Tier 2; 2026-09-14: Tier 0 grew to 137 with #101/#103 and the review follow-ups and Tier H to 361, 722 total; 2026-09-15: StaticData banks and layouts (Tier 0 141, Tier 2 114) and doctor toolchain checks (Tier H 368), 737 total; post-release review follow-ups added a GUID parser test, Tier 0 142, 738 total; see docs/testing.md)
...
|---------|------|------------|
| v0.44.0 | 2026-09-15 | **First release on 4.1.1.7398727** (first release since v0.39.0) — #101/#103 integrated with credit (@mikowals: PersistentVars saved as `null`, `Ext.Enums.X[n]` nil, ARM64 hook at target+4, Osiris return values discarded; 23 mutation-hardening tests), JSON proxy serialization reworked with a cycle guard; offline offset migration to 7398727 verified live: tier 1 114/114, tier 2 112/114 (environmental damage probe + gated ValueList insert), MCM 27/27; five live-found defects fixed (SpellMeta stride 0x60, `entity.Uuid` TypeIndex sync, component GUID byte order, `Transform` field order, UUID-map size guard); `Ext.StaticData` banks resolved through the headmaster table for all types with live-measured strides (every type but ActionResource had been reading a TypeId global), GUID text canonical, new `CharacterCreationAppearanceVisual` type with typed layouts (#100); configure-time ObjC++ toolchain probe + `doctor` toolchain checks (#88); release zip with installer (`scripts/release/`). 737 tests (141 C + 368 pytest + 114 Tier 1 + 114 Tier 2) |
