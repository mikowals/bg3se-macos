...
void lua_ext_register_global_helpers(lua_State *L) {
    ...
    static const char *console_cmd_test_parity_behavior =
        "BG3SE_AddTest(1, 'Debug.ModErrors', function()\n"
        "  -- A handler error is kept with its traceback, newest first.\n"
        "  Ext.Debug.ClearModErrors()\n"
        "  local id = Ext.Events.Log:Subscribe(function(e)\n"
        "    if tostring(e.Message):find('BG3SE_ModErrorsProbe') then\n"
        "      error('BG3SE_ModErrorsProbe raised')\n"
        "    end\n"
        "  end)\n"
        "  Ext.Print('BG3SE_ModErrorsProbe')\n"
        "  Ext.Events.Log:Unsubscribe(id)\n"
        "  local errs = Ext.Debug.ModErrors()\n"
        "  assert(#errs >= 1, 'no error captured')\n"
        "  local e = errs[1]\n"
        "  assert(e.Error:find('BG3SE_ModErrorsProbe raised', 1, true), 'message: ' .. e.Error)\n"
        "  assert(e.Error:find('stack traceback', 1, true), 'no traceback: ' .. e.Error)\n"
        "  AssertType(e.Mod, 'string', 'Mod')\n"
        "  AssertType(e.Time, 'number', 'Time')\n"
        "  AssertEquals(#Ext.Debug.ModErrors(e.Mod), #errs, 'filter by the same mod')\n"
        "  AssertEquals(#Ext.Debug.ModErrors('BG3SE_NoSuchMod'), 0, 'filter by another mod')\n"
        "  Ext.Debug.ClearModErrors()\n"
        "  AssertEquals(#Ext.Debug.ModErrors(), 0, 'cleared')\n"
        "end)\n"
