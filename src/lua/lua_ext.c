...
void lua_ext_register_global_helpers(lua_State *L) {
    ...
    static const char *console_cmd_test_wave3_damage_events =
        ...
        "    local p = e.Params\n"
        "    if BG3SE_DamageEventProbe.attack or not p or p.Type ~= 'AttackTarget'\n"
        "      or not p.Attack or (p.Attack.TotalDamageDone or 0) <= 0 then return end\n"
        "    local sum, types = 0, {}\n"
        ...
        "-- happen on an earlier tick): the host makes one main-hand attack on the\n"
        "-- nearest other party member. Returns the target or nil.\n"
        "function BG3SE_PrimeDamageProbe()\n"
        "  if BG3SE_DamageEventProbe.before > 0 then return 'already' end\n"
        "  local h = Osi.GetHostCharacter()\n"
        "  if not h then return nil end\n"
        "  local best, bd = nil, 1e9\n"
        "  for _, row in ipairs(Osi.DB_Players:Get(nil)) do\n"
        "    local c = row[1]\n"
        "    if c:sub(-36) ~= h:sub(-36) then\n"
        "      local d = Osi.GetDistanceTo(h, c)\n"
        "      if d and d < bd then best, bd = c, d end\n"
        "    end\n"
        "  end\n"
        ...
        "  AssertNotNil(a, 'no damaging AttackTarget functor observed; run BG3SE_PrimeDamageProbe() first')\n"
        "  AssertEquals(a.caster, tostring(Ext.Entity.Get(Osi.GetHostCharacter())), 'Params.Caster is the attacker')\n"
        "  assert(a.target:match('^Entity%('), 'Params.Target is an entity: ' .. a.target)\n"
