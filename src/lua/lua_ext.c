...
void lua_ext_register_global_helpers(lua_State *L) {
    ...
    static const char *console_cmd_test_ingame =
        ...
        "end)\n"
        "BG3SE_AddTest(2, 'Debug.ScanArrayStride.SpellBook', function()\n"
        "  -- SpellBook.Spells shipped the Windows stride 88; the ARM64 stride is 0x68.\n"
        "  local r = Ext.Debug.ScanArrayStride(Ext.Entity.Get(Osi.GetHostCharacter()).SpellBook, 'Spells')\n"
        "  AssertEquals(r.Verdict, 'PASS', 'SpellBook.Spells stride: ' .. tostring(r.Reason))\n"
        "  AssertEquals(r.Suggested, 0x68, 'suggested stride')\n"
        "  local ok = pcall(Ext.Debug.ScanArrayStride, {}, 'Spells')\n"
        "  AssertEquals(ok, false, 'a non-proxy argument is an error')\n"
        "end)\n"
