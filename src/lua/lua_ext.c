...
void lua_ext_register_global_helpers(lua_State *L) {
    ...
    static const char *console_cmd_test_wave3_damage_events =
        ...
        "-- party member. Returns the target or nil.\n"
        "-- Any prompt waits for a click and blocks the cast. A weapon attack can\n"
        "-- prompt the attacker (a barbarian's Reckless Attack reroll on a low roll)\n"
        "-- or the target (Shield); a saving-throw cantrip has no attack roll. The\n"
        "-- target can still prompt (Counterspell, a ward), so prefer the member with\n"
        "-- the fewest such spells, then the nearest; each call tries a new member.\n"
        ...
        "  -- e.Params of the probe's cast, recorded above.\n"
        ...
        "  AssertEquals(a.spell, 'Target_ViciousMockery', 'Params.SpellId.OriginatorPrototype')\n"
