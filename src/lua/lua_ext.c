...
void lua_ext_register_global_helpers(lua_State *L) {
    ...
    static const char *console_cmd_test_ingame =
        ...
        "end)\n"
        "BG3SE_AddTest(2, 'Entity.MeasuredLayouts', function()\n"
        "  -- EncumbranceStats: three ascending thresholds that grow with Strength.\n"
        "  local rows = {}\n"
        "  for _, row in ipairs(Osi.DB_Players:Get(nil)) do\n"
        "    local es = Ext.Entity.Get(row[1]).EncumbranceStats\n"
        "    AssertNotNil(es, 'EncumbranceStats')\n"
        "    assert(0 < es.UnencumberedWeight and es.UnencumberedWeight < es.EncumberedWeight\n"
        "      and es.EncumberedWeight < es.HeavilyEncumberedWeight, 'thresholds ascend')\n"
        "    rows[#rows + 1] = {Osi.GetAbility(row[1], 'Strength'), es.UnencumberedWeight}\n"
        "  end\n"
        "  table.sort(rows, function(a, b) return a[1] < b[1] end)\n"
        "  for i = 2, #rows do\n"
        "    assert(rows[i][1] == rows[i-1][1] or rows[i][2] > rows[i-1][2], 'weight grows with Strength')\n"
        "  end\n"
        "  -- Progression sub-entities: passive names are stats; improvements are +0..+2.\n"
        "  local passives, improved = 0, 0\n"
        "  for _, e in ipairs(Ext.Entity.GetAllEntitiesWithComponent('eoc::progression::PassivesComponent')) do\n"
        "    for i = 1, #e.ProgressionPassives.AddPassives do\n"
        "      local n = e.ProgressionPassives.AddPassives[i]\n"
        "      AssertNotNil(Ext.Stats.Get(n), 'progression passive is a stat: ' .. tostring(n))\n"
        "      passives = passives + 1\n"
        "    end\n"
        "  end\n"
        "  for _, e in ipairs(Ext.Entity.GetAllEntitiesWithComponent('eoc::progression::AbilityImprovementsComponent')) do\n"
        "    local imp = e.AbilityImprovements.AbilityImprovements\n"
        "    for i = 1, 7 do\n"
        "      assert(imp[i] >= 0 and imp[i] <= 2, 'improvement out of range: ' .. tostring(imp[i]))\n"
        "      if imp[i] > 0 then improved = improved + 1 end\n"
        "    end\n"
        "  end\n"
        "  assert(passives > 0, 'no progression passives read')\n"
        "  assert(improved > 0, 'no ability improvements read')\n"
        "end)\n"
