...
void lua_ext_register_global_helpers(lua_State *L) {
    ...
    static const char *console_cmd_test_ingame =
        ...
        "end)\n"
        "BG3SE_AddTest(2, 'Entity.SpellBook.EveryElementIsASpell', function()\n"
        "  -- A wrong stride reads element [1] correctly and the rest as garbage,\n"
        "  -- so check every element resolves its prototype to a spell name.\n"
        "  local spells = Ext.Entity.Get(Osi.GetHostCharacter()).SpellBook.Spells\n"
        "  assert(#spells >= 2, 'host spellbook too small to check a stride: ' .. #spells)\n"
        "  for i = 1, #spells do\n"
        "    local name = Ext.Debug.ReadFixedString(tonumber(spells[i].__ptr))\n"
        "    assert(name and Ext.Stats.Get(name) ~= nil,\n"
        "      'Spells[' .. i .. '] is not a spell (' .. tostring(name) .. '): stride?')\n"
        "  end\n"
        "end)\n"
        "BG3SE_AddTest(2, 'Entity.HashMapCountsAreSizesNotBuckets', function()\n"
        "  -- The bucket count beside a HashMap's size is always a power of two;\n"
        "  -- reading it made every party member's count a power of two.\n"
        "  local function pow2(n) return n > 0 and (n & (n - 1)) == 0 end\n"
        "  for _, field in ipairs({{'StatusContainer', 'StatusCount'},\n"
        "                          {'ActionResources', 'ResourceTypeCount'}}) do\n"
        "    local all_pow2, n = true, 0\n"
        "    for _, row in ipairs(Osi.DB_Players:Get(nil)) do\n"
        "      local c = Ext.Entity.Get(row[1])[field[1]]\n"
        "      local v = c and c[field[2]]\n"
        "      if v then n = n + 1; if not pow2(v) then all_pow2 = false end end\n"
        "    end\n"
        "    assert(n >= 2, field[2] .. ': need 2+ party members, got ' .. n)\n"
        "    assert(not all_pow2, field[2] .. ' is a power of two for every party member: bucket count?')\n"
        "  end\n"
        "end)\n"
    ...
    static const char *console_cmd_test_wave2_components =
        ...
        "end)\n"
        "BG3SE_AddTest(2, 'Entity.UseComponentBoostsMatchStats', function()\n"
        "  -- The arrays sat 8 bytes early and read a non-pointer as the buffer.\n"
        "  local function entries(s)\n"
        "    local n = 0\n"
        "    for part in tostring(s or ''):gmatch('[^;]+') do\n"
        "      if part:match('%S') then n = n + 1 end\n"
        "    end\n"
        "    return n\n"
        "  end\n"
        "  local checked = 0\n"
        "  for _, row in ipairs(Osi.DB_Players:Get(nil)) do\n"
        "    local item = Osi.GetEquippedItem(row[1], 'Melee Main Weapon')\n"
        "    local use = item and Ext.Entity.Get(item).UseComponent\n"
        "    local st = item and Ext.Stats.Get(Osi.GetStatString(item))\n"
        "    if use and st then\n"
        "      AssertEquals(#use.BoostsOnEquipMainHand, entries(st.BoostsOnEquipMainHand),\n"
        "        Osi.GetStatString(item) .. ' BoostsOnEquipMainHand')\n"
        "      checked = checked + 1\n"
        "    end\n"
        "  end\n"
        "  assert(checked >= 1, 'no party weapon with a UseComponent to check')\n"
        "end)\n"
        "BG3SE_AddTest(2, 'Entity.WindowsComponentNames', function()\n"
        "  -- Windows ExtComponentType names ('Health') returned 0 entities.\n"
        "  local byWin = #Ext.Entity.GetAllEntitiesWithComponent('Health')\n"
        "  local byEngine = #Ext.Entity.GetAllEntitiesWithComponent('eoc::HealthComponent')\n"
        "  assert(byEngine > 0, 'no entities with eoc::HealthComponent')\n"
        "  AssertEquals(byWin, byEngine, \"GetAllEntitiesWithComponent('Health')\")\n"
        "  local e = Ext.Entity.Get(Osi.GetHostCharacter())\n"
        "  AssertNotNil(e:GetComponent('Health'), \"entity:GetComponent('Health')\")\n"
        "end)\n"
        "BG3SE_AddTest(2, 'Entity.TransformBothShapes', function()\n"
        "  -- Windows: entity.Transform.Transform.Translate[1..3];\n"
        "  -- macOS legacy: entity.Transform.Translate.x/y/z. Both must agree with Osi.\n"
        "  local host = Osi.GetHostCharacter()\n"
        "  local t = Ext.Entity.Get(host).Transform\n"
        "  local x, y, z = Osi.GetPosition(host)\n"
        "  local win = t.Transform.Translate\n"
        "  for i, want in ipairs({x, y, z}) do\n"
        "    assert(math.abs(win[i] - want) < 0.01, 'Transform.Transform.Translate[' .. i .. ']')\n"
        "  end\n"
        "  assert(math.abs(t.Translate.x - x) < 0.01 and math.abs(t.Translate.z - z) < 0.01,\n"
        "    'Transform.Translate.x/z')\n"
        "  AssertEquals(#t.Transform.RotationQuat, 4, 'RotationQuat has 4 components')\n"
        "end)\n"
        "BG3SE_AddTest(2, 'Entity.FullNameIndexMatchesShortName', function()\n"
        "  local e = Ext.Entity.Get(Osi.GetHostCharacter())\n"
        "  local full = e['eoc::status::ContainerComponent']\n"
        "  AssertNotNil(full, \"entity['eoc::status::ContainerComponent']\")\n"
        "  AssertEquals(full.StatusCount, e.StatusContainer.StatusCount, 'same component either way')\n"
        "end)\n"
        "BG3SE_AddTest(2, 'Entity.FixedStringFieldsAreStrings', function()\n"
        "  -- Component FixedString fields returned the raw index (1125122112\n"
        "  -- for LevelName); cross-check against Osiris's own level database.\n"
        "  local level = Ext.Entity.Get(Osi.GetHostCharacter()).Level\n"
        "  AssertNotNil(level, 'host Level component')\n"
        "  AssertType(level.LevelName, 'string', 'Level.LevelName')\n"
        "  AssertEquals(level.LevelName, Osi.DB_CurrentLevel:Get(nil)[1][1],\n"
        "    'Level.LevelName vs DB_CurrentLevel')\n"
        "end)\n"
