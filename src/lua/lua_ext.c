...
void lua_ext_register_global_helpers(lua_State *L) {
    ...
    static const char *console_cmd_test_ingame =
        ...
        "end)\n"
        "-- Run before !test_ingame (events arrive on later ticks): subscribes,\n"
        "-- applies BLESS to the host and removes it 1.5 s later.\n"
        "function BG3SE_PrimeStatusProbe()\n"
        "  if BG3SE_StatusEventProbe then return 'already' end\n"
        "  local host = Osi.GetHostCharacter()\n"
        "  if not host then return nil end\n"
        "  local hh = tonumber(tostring(Ext.Entity.Get(host)):match('0x(%x+)'), 16)\n"
        "  local p = {applied = 0, removed = 0}\n"
        "  BG3SE_StatusEventProbe = p\n"
        "  Ext.Events.StatusApplied:Subscribe(function(e)\n"
        "    if e.StatusId == 'BLESS' and e.Entity == hh then p.applied = p.applied + 1 end\n"
        "  end)\n"
        "  Ext.Events.StatusRemoved:Subscribe(function(e)\n"
        "    if e.StatusId == 'BLESS' and e.Entity == hh then p.removed = p.removed + 1 end\n"
        "  end)\n"
        "  Ext.Timer.WaitFor(500, function()\n"
        "    Osi.ApplyStatus(host, 'BLESS', 6.0, 1, host)\n"
        "    Ext.Timer.WaitFor(1500, function() Osi.RemoveStatus(host, 'BLESS', host) end)\n"
        "  end)\n"
        "  return host\n"
        "end\n"
        "BG3SE_AddTest(2, 'Events.StatusAppliedRemoved', function()\n"
        "  local p = BG3SE_StatusEventProbe\n"
        "  AssertNotNil(p, 'run BG3SE_PrimeStatusProbe() first')\n"
        "  AssertEquals(p.applied, 1, 'StatusApplied BLESS on the host')\n"
        "  AssertEquals(p.removed, 1, 'StatusRemoved BLESS on the host')\n"
        "end)\n"
